...
예) 20 ether를 받은 컨트랙트가 어떤 로직을 실행하게 할 수 있습니다.

3. 정의하지 않은 함수가 호출될 때, 어떠한 행동을 취하게 할 수 있다. 
예) A 컨트랙트의 a 함수를 호출했는데, A 컨트랙트에 a 함수가 존재하지 않으면, fallback 함수가 호출됩니다.
...
receive:  순수하게 이더만 받을 때 동작 합니다. 
예) call을 이용해서 함수는 호출하지 않고 1 ether를 보낼 때

fallback: 함수가 호출되면서 이더도 받을 때, 불려진 함수가 없을 때 작동합니다.
예) call을 이용해서 a 함수를 호출하면서 1 ether를 보낼 때
